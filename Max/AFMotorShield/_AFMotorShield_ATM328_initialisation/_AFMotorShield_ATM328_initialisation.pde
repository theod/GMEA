/*
 _AFMotorShield_ATM328_initialisation.pde
 
 pour initialiser l'EEPROM d'une arduino avec un 
 AtMega328 et MotorShield Adafruit afin de controller 
 jusqu'à 4 moteurs DC + 2 servo
...
void setEEPROM()
{
  ...
  for(d=0; d<NB_DEV; d++)
  {
    if(d<4) EEPROM.write(_DEV + d, DC);
    else if (d<6) EEPROM.write(_DEV + d, SERVO);
    else EEPROM.write(_DEV + d, STEPPER);
